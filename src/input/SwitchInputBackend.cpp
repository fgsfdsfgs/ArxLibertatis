...
static const Keyboard::Key padToArxKey[SDL_CONTROLLER_BUTTON_MAX] = {
	Keyboard::Key_Enter,      // SDL A, Switch B
	Keyboard::Key_Backspace,  // SDL B, Switch A
	Keyboard::Key_Spacebar,   // SDL X, Switch Y
	Keyboard::Key_Tab,        // SDL Y, Switch X
	...
	, numFingers(0)
{
	...
	if(m_pad)
		...
		LogWarning << "Failed to open controller: " << (controllername != NULL ? controllername : "NULL");
	
	arx_assert(m_pad != NULL);
	...
	std::fill_n(currentAxis, boost::size(currentAxis), 0.0f);
	std::fill_n(axisScale, boost::size(axisScale), 0.25f);
...
void SDL2InputBackend::onEvent(const SDL_Event & event) {
	...
	switch(event.type) {
		...
		case SDL_CONTROLLERBUTTONUP: {
			...
			if(key < int(boost::size(padToArxKey)) && padToArxKey[key] != Keyboard::Key_Invalid) {
